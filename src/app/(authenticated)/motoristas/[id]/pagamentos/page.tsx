    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {pagamentos.length > 0 && (
        <div className="bg-[rgb(var(--card))] border border-[rgb(var(--border))] rounded-xl p-6">
          ...
          <div className="space-y-2">
            {pagamentos.map(p => (
              <div key={p.id} className="flex items-center justify-between p-3 bg-[rgb(var(--muted))] rounded-lg">
                <div>
                  <p className="text-sm font-medium">R$ {p.valor.toFixed(2)}</p>
                  <p className="text-xs text-[rgb(var(--muted-foreground))]">{p.data} {p.descricao && `— ${p.descricao}`}</p>
                </div>
      ...
      {pagamentos.length === 0 && (
        <div className="bg-[rgb(var(--card))] border border-[rgb(var(--border))] rounded-xl p-12 text-center">
          <p className="text-[rgb(var(--muted-foreground))]">Nenhum pagamento registrado</p>
        </div>
      )}
    </div>
  )
